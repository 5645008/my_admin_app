...
const AdminDangerMapPage = () => {
  ...
  const handleDeletePath = async (id) => {
    ...
    try {
      await axios.delete(`http://moyak.store:3001/api/deletecomplaintsmap/${id}`);
  ...
  useEffect(() => {
    const fetchRoutes = async () => {
      try {
        const res = await axios.get('http://moyak.store:3001/api/complaintsmap');
        const updatedPaths = [];

        for (const path of res.data) {
          if (!path.route_coords) {
            try {
              const registerRes = await axios.post('http://moyak.store:3001/api/router/register', {
